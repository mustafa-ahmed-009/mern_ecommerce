            ...
            <div className="bg-gray-100 p-4 flex flex-wrap justify-between items-center border-b">
              <div>
                 <p className="text-sm font-semibold text-gray-700">
                  Order ID: <span className="text-gray-900">{order._id}</span> {/* Use required _id */}
                 </p>
               ...
               <div className="md:col-span-2">
                  ...
                  <ul className="space-y-3">
                     ...
                     {order.cartItems.map((item: CartItem) => (
                       ...
                       <li key={item._id} className="flex items-start sm:items-center space-x-3 border-b pb-3 last:border-b-0">
                           {/* Use item.image */}
                           <img
                             ...
                             onError={(e) => (e.currentTarget.src = '/placeholder.png')} // Handle broken image links
                           />
                           <div className="flex-grow">
                              {/* Use item.title */}
                              <p className="font-medium text-gray-800">{item.title || 'Product Name Missing'}</p>
                           ...
                           <p className="text-sm font-semibold text-gray-800 flex-shrink-0">{formatCurrency(item.price * item.quantity)}</p>
                       </li>
                     ))}
               ...
               <div className="md:col-span-1 space-y-4 border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-4">
                  <div>
                      ...
                      <div className="text-sm space-y-1">
                          ...
                          <p className="flex justify-between"><span>Subtotal:</span> <span>{formatCurrency(order.totalAmount - order.shippingCost)}</span></p>
                          <p className="flex justify-between"><span>Shipping:</span> <span>{formatCurrency(order.shippingCost)}</span></p>
                          <p className="flex justify-between font-bold text-base border-t pt-1 mt-1"><span>Total:</span> <span>{formatCurrency(order.totalAmount)}</span></p>
                      </div>
                  </div>
                  <div>
                      <h3 className="text-lg font-semibold mb-2">Shipping Address</h3>
                      <address className="text-sm text-gray-700 space-y-1 not-italic"> {/* Use <address> tag */}
                         <p>{order.shippingAddress.street}{order.shippingAddress.details ? `, ${order.shippingAddress.details}` : ''}</p>
                         <p>{order.shippingAddress.governorate}, {order.shippingAddress.country}</p>
