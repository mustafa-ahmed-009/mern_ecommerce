...
export const OrdersService = {
  addOrder: createAsyncThunk(
    ...
    async (
      orderData: Omit<OrderModel, "_id" | "createdAt" | "updatedAt" | "__v">,
      { rejectWithValue },
    ) => {
      // More specific input type
      try {
        // Assuming API takes order data and returns the created order { data: OrderModel }
        const response = await axiosInstance.post<{ data: OrderModel }>(
          "orders",
          orderData,
        );
  ...
  getAllOrders: createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
      try {
        ...
        const response = await axiosInstance.get<{ data: OrderModel[] }>(
          "orders",
        );
        return response.data; // Payload will be { data: OrderModel[] }
      } catch (error: any) {
