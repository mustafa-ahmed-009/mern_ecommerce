...
export const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        OrdersService.addOrder.fulfilled,
        (state, action: PayloadAction<{ data: OrderModel }>) => {
          state.loading = false;
      ...
      // Assuming payload is { data: OrderModel[] }
      .addCase(
        OrdersService.getAllOrders.fulfilled,
        (state, action: PayloadAction<{ data: OrderModel[] }>) => {
          state.loading = false;
      ...
      // Assuming payload is { data: OrderModel[] }
      .addCase(
        OrdersService.getOrderById.fulfilled,
        (state, action: PayloadAction<{ data: OrderModel[] }>) => {
          state.loading = false;
          state.orders = action.payload.data; // *** Correctly replace the list ***
        },
      ...
      .addCase(OrdersService.updateOrderStatus.pending, (state) => {
      ...
      .addCase(
        OrdersService.updateOrderStatus.fulfilled,
        (state, action: PayloadAction<{ data: OrderModel }>) => {
          state.loading = false;
