...
const ProductsPage = () => {
  ...
  const initialProductFromState = location.state?.product as Product | undefined;
  // State to hold the fully loaded product data
  const [currentProduct, setCurrentProduct] = useState<Product | null>(initialProductFromState || null);
  // State to track loading status for the *specific* product fetch
  const [isLoadingProduct, setIsLoadingProduct] = useState<boolean>(!initialProductFromState); // Start loading if no initial data passed
  ...
  useEffect(() => {
    ...
    const shouldFetch = productId && (!initialProductFromState || initialProductFromState._id !== productId);
    ...
    if (shouldFetch) {
      const fetchProduct = async () => {
        ...
        try {
          ...
          const fetchedProduct = await dispatch(ProductsService.fetchSingleProduct(productId)).unwrap();
          setCurrentProduct(fetchedProduct); // Update local state with fetched data
        } catch (err: any) {
          // Handle errors from the fetch operation
          console.error("Failed to fetch product:", err);
          const errorMessage = typeof err === 'string' ? err : err?.message || 'Failed to load product details.';
  ...
  // --- Add to Cart Handler ---
   const handleAddToCart = async () => {
     ...
     if (!userState) { // Check if user is logged in
     ...
     try {
       ...
       await dispatch(CartService.addAnItemToTheCart(currentProduct._id)).unwrap();
       toast.success(`${currentProduct.title} added to cart!`);
     } catch (err: any) {
       // Handle potential errors during cart addition
       console.error('Failed to add to cart:', err);
       const errorMessage = typeof err === 'string' ? err : err?.message || 'Could not add product to cart.';
  ...
  // --- Wishlist Handlers ---
   const handleAddToWishlist = async () => {
     ...
     if (!userState) { toast.error('Please log in to manage your wishlist.'); return; }
      try {
       await dispatch(UserService.addPrdouctToWishList(currentProduct._id)).unwrap();
       toast.success('Added to wishlist!');
     } catch (err: any) {
         console.error("Failed to add to wishlist:", err);
         const errorMessage = typeof err === 'string' ? err : err?.message || 'Could not add to wishlist.';
         toast.error(errorMessage);
      }
   };
   ...
   const handleRemoveFromWishlist = async () => {
     if (!currentProduct) return;
     if (!userState) { toast.error('Please log in.'); return; }
     try {
       await dispatch(UserService.removeProductFromWishList(currentProduct._id)).unwrap();
       toast.success('Removed from wishlist.'); // Using success for consistency
     } catch (err: any) {
         console.error("Failed to remove from wishlist:", err);
         const errorMessage = typeof err === 'string' ? err : err?.message || 'Could not remove from wishlist.';
         toast.error(errorMessage);
     }
   };
  ...
  if (!currentProduct) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
         ...
         <p className="text-gray-600">The product details could not be loaded or the product does not exist.</p>
         <button
        ...
        <div className="w-full lg:w-1/2 flex flex-col">
          ...
          {currentProduct.category?._id && currentProduct.category?.name && (
            <span
              ...
              onClick={() => navigate(`/category/${currentProduct.category._id}`)}
            >
          ...
          <p className="text-3xl font-semibold text-red-600 mb-5">
             ...
             {currentProduct.priceAfterDiscount && currentProduct.priceAfterDiscount < currentProduct.price
          ...
          {/* Stock Availability */}
          <p className={`mb-6 text-sm font-medium ${currentProduct.quantity > 0 ? 'text-green-600' : 'text-red-600'}`}>
            {currentProduct.quantity > 0 ? `In stock (${currentProduct.quantity} available)` : 'Out of Stock'}
          </p>
          ...
          {currentProduct.quantity > 0 && (
              <div className="flex items-center gap-4 mb-6">
                ...
                {/* Wishlist Toggle Button */}
                 <button
                    onClick={() => userState?.wishlist.includes(currentProduct._id)
                    ...
                    title={userState?.wishlist.includes(currentProduct._id) ? "Remove from Wishlist" : "Add to Wishlist"}
                    aria-label={userState?.wishlist.includes(currentProduct._id) ? "Remove from Wishlist" : "Add to Wishlist"}
                 >
      ...
      {relatedProducts.length > 0 && (
        <div className="pt-8 border-t mt-12">
          ...
          {loadingList ? (
              ...
              <ErrorMessage message={`Could not load related products: ${errorList}`} />
          ) : (
            <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
              ...
              {relatedProducts.map((relatedProduct) => (
                  // Recommended: Replace inline card with <ProductCard product={relatedProduct} key={relatedProduct._id} />
                   <div key={relatedProduct._id} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden flex flex-col group p-3 text-center">
                       <div className="relative cursor-pointer overflow-hidden mb-2 group" onClick={() => navigate(`/products/${relatedProduct._id}`)}>
                           <img src={relatedProduct.imageCover} alt={relatedProduct.title} className="w-full h-40 object-cover rounded transition-transform duration-300 group-hover:scale-105"/>
                       </div>
                       <h3 className="text-sm font-semibold text-gray-700 mb-1 truncate hover:text-blue-600 cursor-pointer h-10 flex items-center justify-center" title={relatedProduct.title} onClick={() => navigate(`/products/${relatedProduct._id}`)}>
