...
const HeroSlider: React.FC = () => {
  const settings = {
    ...
    cssEase: "linear",
    appendDots: (
      dots: React.ReactNode, // Custom dots positioning
    ) => (
      <div style={{ position: "absolute", bottom: "20px", width: "100%" }}>
        <ul style={{ margin: "0px", padding: "0", textAlign: "center" }}>
          {" "}
          {dots}{" "}
        </ul>
      </div>
  ...
  return (
    <div className="relative h-[40vh] md:h-[50vh] overflow-hidden rounded-lg shadow-lg mb-8">
      {" "}
      {/* Adjust height and add margin */}
      <style>{`
      ...
      <Slider {...settings}>
        {sliderImages.map((imgUrl, index) => (
          <div key={index} className="h-[50vh] md:h-[50vh]">
            {" "}
